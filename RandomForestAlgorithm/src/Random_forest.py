...
def measure(y_actual, y_predict):
    TP = 0
    FP = 0
    TN = 0
    FN = 0
...
try:
    LogMessage('Health predictions training and test algorithm start ...', 'OK')
      
    # Check if directory exists, or create it
    try:
      os.makedirs(ModelFolderPath)
      LogMessage('Created directory : ' + os.path.realpath(ModelFolderPath), 'OK')
    except FileExistsError:
      # Directory already exists
      LogMessage('Directory already exists : ' + os.path.realpath(ModelFolderPath), 'OK')
      pass
    ...
    LogMessage('Dataset loading complete', 'OK')
    ...
    LogMessage('Encoding labels complete', 'OK')
    ...
    LogMessage('Values definition for random forest classification setup, complete', 'OK')
    ...
    LogMessage('Defining training and test data complete', 'OK')
    ...
    LogMessage('Training and test complete', 'OK')
    ...
    LogMessage('Models saved : ' + os.path.realpath(ModelFolderPath) + '/' + LamenessModelName + '.pkl, ' + 
    ...
    LogMessage('Accuracy score values set complete', 'OK')
    ...
    LogMessage('Output dataset construction complete', 'OK')
  
    # Metrics calculation
    Lameness_TP, Lameness_FP, Lameness_TN, Lameness_FN, Lameness_FPR, Lameness_TPR = measure(Lameness_y_test, Lameness_y_pred)
    Ketosis_TP, Ketosis_FP, Ketosis_TN, Ketosis_FN, Ketosis_FPR, Ketosis_TPR = measure(Ketosis_y_test, Ketosis_y_pred)
    Mastitis_TP, Mastitis_FP, Mastitis_TN, Mastitis_FN, Mastitis_FPR, Mastitis_TPR = measure(Mastitis_y_test, Mastitis_y_pred)
    ...
    LogMessage('Metrics calculation complete', 'OK')
  
    # !!! OUTPUT  DATASET PREDICTIONS AND DATASET METRICS
    dsPredictions = df10
    dsMetrics = pd.DataFrame(Mydict)
  
    LogMessage('Health predictions training and test algorithm complete successfully', 'OK')
    ...
    LogMessage("Unexpected error at line number " + errline + " : " + str(sys.exc_info()), 'ERROR')
